...
public class Tree<E> implements Iterable<Position<E>> {
    ...
    private void displayTree1(Node<E> node, String parentNumber, int childNumber) {
        String nodeNumber = parentNumber + childNumber;
        System.out.println(nodeNumber + ". " + node.getElement());
    ...
    public void displayTree2() {
        displayTree2(root, "", false);
    }
    
    private void displayTree2(Node<E> node, String prefix, boolean isLeft) {
        if (node != null) {
            System.out.println(prefix + (isLeft ? "|---" : "L__") + node.getElement());
    
            List<Node<E>> children = node.getChildren();
            for (int i = 0; i < children.size(); i++) {
                displayTree2(children.get(i), prefix + (isLeft ? "|   " : "    "), i != children.size() - 1);
            }
        }
    }
    /**
